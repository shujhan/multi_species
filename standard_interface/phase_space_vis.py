...
#         p.set_ec('black')
    ...
    plt.savefig(sim_dir_str + f'phase_space_{species}_{simtime:.2f}.png')
    plt.close()
...
def phase_movie(sim_dir, simulation_dictionary,species,do_show_panels,flim=(0,.3), simulation_has_run=True, can_do_movie=True):
    ...
    with writer.saving(fig, f"{sim_dir_str}{panel_string}phase_space_{species}.mp4", dpi=100):
...
#             panels_fs[ii] = 1./36. * np.dot(panel_fs,weights)
        ...
        for iter_num in range(sd['diag_period'], sd['num_steps'] + 1, sd['diag_period']):
            cb.remove()
            # ax.collections.pop()
            # del ax.collections[-1]  # Deletes the last collection (the heatmap)
